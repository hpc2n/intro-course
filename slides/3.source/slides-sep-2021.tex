...
\texttt{\#SBATCH -A SNIC2021-22-514} \\
...
\frame{\frametitle{Important information}
  ...
  \begin{block}{}
    \begin{itemize}
    \item The course project has the following SNIC ID: SNIC2021-22-514
    \item In order to use it in a batch job, add this to the batch script:
      \begin{itemize}
        \item \#SBATCH -a SNIC2021-22-514
      \end{itemize}
    \item There are two nodes reserved for the course, in order to let
      us run small examples without having to wait for too long. One
      node is a regular CPU node, the other a GPU node. 
    \item These reservations are ONLY valid during the course:
      \begin{itemize}
      \item intro-hpc \\ (add with \#SBATCH --reservation=intro-hpc)
      \item intro-hpc-gpu \\ (add with \#SBATCH --reservation=intro-hpc-gpu)
      \end{itemize}
\item We have a storage project linked to the compute project. It is snic2021-22-514. You find it in /proj/nobackup/snic2021-22-514
    \end{itemize}
  \end{block}
